...
export function useMagnifierStyle(
  ...
  function getMappingArea(imgRect: DOMRect) {
    ...
    return {
      top: imgRect.top > verticalPadding
        ? imgRect.top
        : imgRect.top + verticalPadding,
      bottom: Math.abs(imgRect.top) > verticalPadding
        ? imgRect.top + imgRect.height
        : imgRect.top + imgRect.height - verticalPadding,
  ...
  /**
   * 計算滑鼠相對於映射區域的位置（百分比）
   * 注意：position 是 useMagnifierEvents 的 updateNormalPosition 的包含 sensitivity 的結果
   */
